...
public class SocketIOServer_Yiki {
...
//            });
         ...
         */
        server.addEventListener("login", Result.class, new DataListener<Result>() {
            @Override
            public void onData(SocketIOClient client, Result data, AckRequest ackRequest) {
                System.out.println("接收到客户端login消息：code = " + data.getMsg() + ",body = " + data.getData());
